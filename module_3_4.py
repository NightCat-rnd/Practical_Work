# Домашняя работа по уроку "Дополнительное практическое задание по модулю: "Произвольное число параметров"
...
def single_root_words(root_words, *other_words):
    same_word = []                                  # список который будем возвращать
    current_root_words = root_words.lower()         # копия root_words в нижнем регистре
                                                    # переменная что бы не вызывать функцию lower() в каждой итерации
    for i in range(len(other_words)):
        current_other_words = other_words[i].lower() # приводим переданный аргумент в нижний регистр
        if current_root_words in current_other_words or current_other_words in current_root_words :
